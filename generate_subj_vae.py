...
def trainer(data,labels,learning_rate=1e-3, batch_size=100, num_epoch=75, n_z=10):
    ...
    for epoch in range(num_epoch):
        for iter in range(0,len(data),batch_size):
            # Obtina a batch
            batch = (data[iter:iter+batch_size,:],labels[iter:iter+batch_size,:])
...
num_areas =116
#input_dim = int(0.5*num_areas*(num_areas-1))
input_dim = 784
num_sample = len(sfc)
conn_mat = np.zeros([num_sample,num_areas,num_areas])
ids = np.load(os.path.join(dict_path,'all_ids.npy'))
cnt = 0
labels =np.zeros([num_sample,2])
for key in ids:
    conn_mat[cnt, :, :] = sfc[key]
    labels[cnt,labels_dict[key]-1] = 1
    cnt+=1


feats_vect = np.zeros([num_sample,int(0.5*num_areas*(num_areas-1))])
for i in range(num_sample):
    cnt = 0
    for j  in range(num_areas):
        for k in range(j):
            if np.isnan(conn_mat[i,j,k]):
                conn_mat[i, j, k]=0
            feats_vect[i,cnt] = np.abs(conn_mat[i,j,k])
            cnt+=1


training_vect = feats_vect[:751,:]
test_vect = feats_vect[751:,:]


# Train the model
model = trainer(training_vect[:,:784],labels,learning_rate=1e-4,  batch_size=10, num_epoch=100, n_z=5)
