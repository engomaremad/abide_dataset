#TODO: analyze the local accuracies in the ADOS spectrum classification
import numpy as np
import os
...
feat_history =  np.load(os.path.join(dict_path,'accs_ados_3.npy'))
...
yeo_labels = ['cerebelum','visual','somatomotor','dorsal_attention','ventral_attention','limbic','frontoparietal','default']
...
used_cnt = []
display_mat = np.zeros([len(yeo_labels),len(yeo_labels)])

for i in range(n_feats):
    for j in range(i):
        percntiles_mat[i,j] = percntiles[cnt]
        percntiles_mat[j, i] = percntiles_mat[i,j]
        if percntiles_mat[i,j]>=85:
            display_mat[int(aal2yeo[i]),int(aal2yeo[j])]= display_mat[int(aal2yeo[i]),int(aal2yeo[j])]+1
...
displayed_res[displayed_res>85]=95
displayed_res[0,0] =100
...
cax = ax.imshow(display_mat, interpolation='nearest', cmap=cm.jet)
...
cbar = fig.colorbar(cax, ticks=np.round(np.linspace(0,np.max(display_mat),(np.max(display_mat)/5)+1)))
cbar.ax.set_yticklabels(np.round(np.linspace(0,np.max(display_mat),(np.max(display_mat)/5)+1)),verticalalignment='center')  # vertically oriented colorbar
ax.xaxis.tick_top()
       # inc. width of y-axis and color it red
plt.xticks(range(len(yeo_labels)),yeo_labels,rotation =45,fontsize =12)
plt.yticks(range(len(yeo_labels)),yeo_labels,rotation =0,fontsize =12 )
